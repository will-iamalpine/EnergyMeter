...
{
  emonData.power_real = ct1.realPower;
  emonData.power_apparent = ct1.apparentPower;
  emonData.Vrms = ct1.Vrms;
  emonData.Irms = ct1.Irms;
  emonData.power_factor = ct1.powerFactor;
  emonData.phase_angle = acos(ct1.powerFactor) * (180/(atan(1)*4));
  emonData.power_reactive = sqrt(pow(ct1.apparentPower,2) - pow(ct1.realPower,2));

  byte binarray[sizeof(emonData)];
  memcpy(binarray, &emonData, sizeof(emonData));
